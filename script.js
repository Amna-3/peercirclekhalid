...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedRole) {
        ...
        document.getElementById('login-modal').classList.add('flex'); // Ensure it's visible
    }
});
...
function showMentorEmail(mentorEmail, mentorName) {
    // Use a custom modal for better aesthetics and copy functionality
        ...
        <article class="mentor-card">
            <header class="flex items-center justify-between mb-4">
                <div class="flex items-center">
                    <div class="w-12 h-12 rounded-full bg-gradient-to-br from-amber-100 to-amber-200 dark:from-slate-700 dark:to-slate-600 flex items-center justify-center mr-3 shadow-sm">
                        <i class="fas fa-user-nurse text-amber-700 dark:text-amber-300 text-lg"></i>
                    </div>
                    <div>
                        <h3 class="font-bold text-slate-800 dark:text-white text-base">${escapeHTML(m.name)}</h3>
                        <p class="text-xs text-slate-500 dark:text-slate-400 font-medium">${escapeHTML(m.year)}</p>
                    </div>
                </div>
                <span class="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-sm"></span>
            </header>
            <p class="text-sm text-slate-600 dark:text-slate-300 italic mb-4 leading-relaxed">"${escapeHTML(m.quote)}"</p>
            <div class="flex flex-wrap gap-2 mb-4">
                ${m.tags.map(tag => `<span class="px-3 py-1 text-xs rounded-full bg-gradient-to-r from-amber-50 to-amber-100 dark:from-teal-900/30 dark:to-teal-800/30 text-amber-800 dark:text-teal-300 font-medium border border-amber-200 dark:border-teal-700/50">${tag}</span>`).join('')}
            </div>
            <button onclick="showMentorEmail('${escapeHTML(m.email)}', '${escapeHTML(m.name)}')" class="mentor-email w-full justify-center">
...
function handleBooking(e) {
    ...
    const subject = encodeURIComponent(`PeerCircle Support Request: ${topic} from ${name}`);
    const body = encodeURIComponent(
        `Hello,\n\n` +
        `A student has requested support via PeerCircle:\n\n` +
        `Name: ${name}\n` +
        `Student Email: ${email}\n` +
        `Year of Study: ${year}\n` +
        `Topic: ${topic}\n\n` +
        `Message:\n${message}\n\n` +
        `Please reply directly to the student's email address (${email}) to start the conversation.\n\n` +
        `Best regards,\n` +
        `PeerCircle System`
    );
    
    // 4. OPEN MAILTO LINK
    // This is the most reliable method for a static site to ensure the mentor receives the email.
    if (supportEmail) {
        window.location.href = `mailto:${supportEmail}?subject=${subject}&body=${body}`;
    } else {
        alert("Please enter the email of the person you need support from.");
        return;
    }

    // 5. STORE MESSAGE (for internal dashboard)
    const newMessage = {
    ...
    showNotification("Request Prepared Successfully", "success");
}
...
// Admin credentials (hardcoded for demo - in production, use a backend)
const ADMIN_CREDENTIALS = {
    email: "admin@rakmhsu.ac.ae",
